...
function applyRoleAccess(role) {
    ...
    // 2. Define Groups
    const salesRoles = ["District Sales Manager", "Regional Sales Manager", "Regional Distribution Manager"];
    const coordRoles = ["Zonal Sales Coordinator"];
...
// Fatima Fertilizer Warehouse Data 
...
// Populate Regions when Zone is selected
...
// Populate Warehouses when Region is selected
...
salesForm.addEventListener('submit', function(e) {
    ...
    const uniqueId = "FAT-" + dateString + "-" + randomNum;
...
// --- 4. FETCH AND DISPLAY LIVE DATA ---
const trackerBody = document.getElementById('tracker-body');
const refreshTrackerBtn = document.getElementById('refresh-tracker');

function fetchShipments() {
    // Show a loading message while it fetches from Google Sheets
    ...
    fetch(GOOGLE_SCRIPT_URL)
        ...
        .then(data => {
            if(data.status === "success") {
                trackerBody.innerHTML = ''; // Clear loading text
                
                // Get the rows and reverse them so the newest request is at the top
        ...
        .catch(error => {
            trackerBody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:red;">Error loading data. Check internet connection.</td></tr>';
...
function fetchQueue() {
    ...
    fetch(GOOGLE_SCRIPT_URL)
        ...
        .then(data => {
            if(data.status === "success") {
                ...
                if (pendingShipments.length === 0) {
                    queueBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">No pending requests!</td></tr>';
                    return;
                }
        ...
        .catch(error => {
            queueBody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:red;">Error loading queue.</td></tr>';
        });
}
...
window.assignPlant = function(uid) {
    ...
    if (!selectedPlant) {
        alert("Please select a Plant before assigning!");
        return;
    }
...
function fetchPlantQueue() {
    ...
    fetch(GOOGLE_SCRIPT_URL)
        ...
        .then(data => {
            if(data.status === "success") {
                ...
                if (plantShipments.length === 0) {
                    plantBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">No pending loads at the plant!</td></tr>';
                    return;
                }
                ...
                plantShipments.forEach(shipment => {
                    ...
                    let row = `<tr>
                        ...
                        <td>${qty}</td>
                        <td>
                            <input type="date" id="batch-date-${uid}" style="padding: 6px; font-size: 0.9rem; width: 100%;">
                        </td>
                        <td>
                            <button onclick="dispatchPlant('${uid}')" style="background:#1a5c3a; color:white; border:none; padding:8px 12px; border-radius:4px; font-weight:bold; cursor:pointer;">Dispatch</button>
                        </td>
                    </tr>`;
        ...
        .catch(error => {
            plantBody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:red;">Error loading plant queue.</td></tr>';
        });
}
...
window.dispatchPlant = function(uid) {
    ...
    if (!batchDate) {
        alert("FIFO Rule: Please enter the Batch Production Date before dispatching!");
        return;
    }
